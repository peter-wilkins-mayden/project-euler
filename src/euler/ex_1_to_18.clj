...
(defn sum-divisible-by [target n]
  (let [p (quot target n)]
    (/ (* n p (inc p)) 2)))
;1
#_(apply + (filter #(or (zero? (mod % 3)) (zero? (mod % 5))) (range 1E9)))
;233333333166666668

#_(let [t (- 1E9 1)] (- (+ (sum-divisible-by t 3) (sum-divisible-by t 5)) (sum-divisible-by t 15)))
;2.33333333166666688E17  - rounding error?
...
;3
#_(time (->> (primes)
             (take-while #(< % (Math/sqrt 600851475143.0)))
             (filter #(and (zero? (mod 600851475143 %))))
             last))
;Elapsed time: 4587.075805 msecs

(defn factors [x]
  (loop [x x a 2]
    (cond
      (<= x 1) a
      (zero? (mod x a)) (recur (/ x a) a)
      :else (recur x (inc a)))))

#_(time (factors 600851475143))
;Elapsed time: 4.047404 msecs

(defn palindomic-number? [x]
  (= (seq (str x)) (reverse (str x))))

;4 **
#_(time (->> (for [x (range 999 100 -1) y (range 998 100 -1)] [x y (* x y)])
             (drop-while #(not (palindomic-number? (last %))))
             first))

;5
;(map (fn [x][x (/ (* 19 16 17 13 11 9 7 5) x)]) (range 1 21))
;(map (fn [x][x (/ (* 9 8 7 5) x)]) (range 1 11))

#_(let [limit 20]
    (reduce
      (fn [acc p] (* acc (Math/pow p (int (/ (Math/log limit) (Math/log p))))))
      1
...
#_(let [x 100
        ...
        sum (apply + nums)
        square #(* % %)]
    (- (square sum) (apply + (map square nums))))

#_(let [x 100
        sum (* x (inc x) 1/2)
        square (* (inc (* 2 x)) (inc x) x 1/6)]
    (- (* sum sum) square))
...
;7  robban  *****
#_(time (last (take 10001 (primes))))                       ; 550 ms
#_(time (last (take 10001 (filter is-prime? (range)))))     ; 121 ms

(defn pythagorean-triplet? [x y z]
  (and (< x y z)
       (= (+ (* x x) (* y y)) (* z z))))

;9 *****   https://www.youtube.com/watch?v=QJYmyhnaaek&vl=en
#_(time (for [x (range 333) y (range 500) z (range 500)
              :when (and (< x y z) (= 1000 (+ x y z)) (pythagorean-triplet? x y z))]
          (* x y z)))

;10  ****
#_(time (apply + (take-while #(< % 2E6) (primes))))         ;12750
#_(time (apply + 2 (take-while #(< % 2E6) (filter is-prime? (range 3 (inc 2E6) 2))))) ; 4034

(defn primes-10 [n]
  (let [r (Math/pow n 0.5)
        _ (assert (and (<= (* r r) n) (> (Math/pow (inc r) 2) n)))
        V (range 1 (inc r))]))
...
(let [horz (->> (map (partial partition 4) grid)
                (mapcat identity)
                (map #(apply * %)))
      vert (->> (apply map list grid)
                (map (partial partition 4))
                (mapcat identity)
                (map #(apply * %)))
      diag-r (->> (map-indexed #(drop (mod %1 4) %2) grid)
                  (apply map list)
                  (map (partial partition 4))
                  (mapcat identity)
                  (map #(apply * %)))
      diag-l (->> (map-indexed #(drop ({0 3, 1 2, 2 1, 3 0} (mod %1 4)) %2) grid)
                  (apply map list)
                  (map (partial partition 4))
                  (mapcat identity)
                  (map #(apply * %)))]
  (apply max (concat vert horz diag-l diag-r)))

(defn num-factors [x]
  (* 2 (count (filter #(zero? (mod x %)) (range 2 (inc (int (Math/sqrt x))))))))

;12
(defn count-factors [ps n]
  (loop [n n ps ps cs {}]
    (let [p (first ps)]
      (cond
        (= 1 n) (apply * (map inc (vals cs)))
        (zero? (mod n p)) (recur (/ n p) ps (update cs p (fnil inc 0)))
        :else (recur n (drop 1 ps) cs)))))

#_(time (let [ps (primes)]
          (->> (map #(/ (* % (inc %)) 2) (iterate inc 1))
               (map (juxt identity (partial count-factors ps)))
               (drop-while #(< (second %) 500))
               ffirst)))                                    ;"Elapsed time: 1479.028513 msecs"
...
#_(time (let [ps (primes)
              f (memoize (partial count-factors ps))]
          (->> (iterate inc 1)
               (map (juxt #(/ (* % (inc %)) 2)
                          #(if (even? %)
                             (* (f (/ % 2)) (f (inc %)))
                             (* (f %) (f (/ (inc %) 2))))))
               (drop-while #(< (second %) 500))
               ffirst)))                                    ;"Elapsed time: 826.910891 msecs"
...
#_(time (let [ps (primes)
          ...
          (loop [n 1]
            (if (> (p n) 500)
              (/ (* n (inc n)) 2)
              (recur (inc n))))))                           ;"Elapsed time: 724.786153 msecs"

; 14
(defn Collatz [n]
  (iterate #(if (even? %) (/ % 2) (inc (* 3 %))) n))

(defn Collatz-loop [acc t]
  (loop [n t c 1]
    (cond
      (= n 1) (assoc acc t c)
      (acc n) (assoc acc t (+ (acc n) c))
      :else (recur
...
#_(let [l 999999]
    (key (apply max-key val
                (reduce Collatz-loop {} (range (int (/ l 2)) (inc l))))))

; 15
(let [g 20]
  (reduce #(* %1 (/ (+ g %2) %2)) 1 (range 1 (inc g))))
...
; 17
(defn letter-count [s e]
  (let [m {0 0, 1 3, 2 3, 3 5, 4 4, 5 4, 6 3, 7 5, 8 5, 9 4, 10 3
           11 6, 12 6, 13 8, 14 8, 15 7, 16 7, 17 9, 18 8, 19 8
           20 6, 30 6, 40 5, 50 5, 60 5, 70 7, 80 6, 90 6}]
    (letfn [(f [x] (cond
                     (<= x 20) (m x)
                     (<= x 99) (+ (m (* 10 (quot x 10))) (m (rem x 10)))
                     (= 1000 x) 11
                     (zero? (mod x 100)) (+ (m (int (/ x 100))) 7)
                     (<= x 999) (+ (m (int (/ x 100))) 10 (f (rem x 100)))
                     ))]
      (apply + (map f (range s (inc e)))))))

#_(letter-count 1 1000)
...
(defn max-path [p]
  (apply max
         (reduce
           #(vec (map-indexed
                   (fn [i v]
                     (+ v (max (get %1 (dec i) 0) (get %1 i 0)))) %2))
           p)))
